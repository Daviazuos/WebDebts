    ...
    public class DebtsApplicationService : IDebtsApplicationService
    {
        ...
        public async Task<List<GetDebtResponsiblePartiesResponse>> GetResponsiblePartiesDebts(Guid? responsiblePartyId, int month, int year, Guid userId)
        {
            ...
            if (debtsresponsibleParties.Count > 0) {
                ...
                .Where(response => response != null) // Removes null items generated by the validation
                .ToList();
            }else
            {
        ...
        public async Task<GenericResponse> CreateDraftDebtFromApp(AddDebtFromAppRequest addDebtFromAppRequest, Guid userId)
        {
            ...
            var draftDebt = new DraftDebt();

            if (addDebtFromAppRequest.bankName == "Nubank")
            {
                draftDebt = await ParseNubankNotification(addDebtFromAppRequest.notification, user, addDebtFromAppRequest.user);
            }
            else if (addDebtFromAppRequest.bankName == "Inter")
            {
                draftDebt = await ParseInterNotification(addDebtFromAppRequest.notification, user, addDebtFromAppRequest.user);
            }
        ...
        public async Task<DraftDebt> ParseNubankNotification(string message, User user, string userName)
        {
            var card = new Card();

            if (userName == "Davi")
            {
                card = await _cardRepository.FindByIdAsync(new Guid("22a209e0-4164-4f7b-ab66-3d7e4ab689d7"));
            }
            else if (userName == "Victoria")
            {
                card = await _cardRepository.FindByIdAsync(new Guid("fb3540b1-be87-43d3-b203-582f5f94e71a"));
            }
            else
            {
                return null; // User not recognized, cannot parse
            }
        ...
        public async Task<DraftDebt> ParseInterNotification(string message, User user, string userName)
        {
            var card = new Card();

            if (userName == "Davi")
            {
                card = await _cardRepository.FindByIdAsync(new Guid("a5356091-334b-4ea9-a174-6978eb5ef968"));
            } else if (userName == "Victoria")
            {
                card = await _cardRepository.FindByIdAsync(new Guid("4d8cb947-0a56-4dd8-ba69-e3e1c787736e"));
            }
            else
            {
                return null; // User not recognized, cannot parse
            ...
            var regex = new Regex(@"comprou R\$ ?(\d+[.,]\d{2}) em (.*?)\. .*?final (\d{4})", RegexOptions.IgnoreCase);
            var match = regex.Match(message);

            if (!match.Success)
            {
                regex = new Regex(@"R\$ ?(\d+[.,]\d{2}) em (.*?)(?:\.|$)", RegexOptions.IgnoreCase);
